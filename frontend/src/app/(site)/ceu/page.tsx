  ...
  return (
    <>
      ...
      <Section title="CEU Provider Annual Credential Fee">
        ...
        <div className="mt-6">
          {annual && (
            <CtaButton href={`/store/${annual.slug}`}>
      ...
      <Section surface title="CEU Workshop Endorsement Fees" intro="Each workshop submitted for ABCAC CEU endorsement is subject to a one-time review fee based on total contact hours.">
        <div className="grid gap-5 md:grid-cols-3">
          {tierProducts.map((p) => (
            <div key={p!.slug} className="flex flex-col rounded-xl border border-line bg-bg p-6">
              ...
              <CtaButton href={`/store/${p!.slug}`} variant="outline" size="sm" className="mt-5 self-start">Submit for Endorsement</CtaButton>
            </div>
          ))}
        </div>
