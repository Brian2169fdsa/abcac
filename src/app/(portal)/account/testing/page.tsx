...
export default async function TestingRegistrationPage({ searchParams }: { searchParams: { mode?: string } }) {
  ...
  const [{ data: profile }, { data: requests }] = await Promise.all([
    ...
    admin.from("testing_requests").select("id,exam_code,testing_mode,status,payment_status,smt_candidate_id,submitted_at,preregistered_at").eq("member_id", memberId).order("created_at", { ascending: false }).limit(10),
  ]);
