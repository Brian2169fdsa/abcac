...
export default async function AdminLegacyMembers({ searchParams }: { searchParams: { q?: string } }) {
  ...
  try {
    let query = sb
      .from("legacy_members")
      .select("id,first_name,last_name,email,cert_type,cert_number,expiration_date,invited_at,claimed_by,claimed_at,import_batch")
      .order("last_name", { ascending: true })
      .limit(200);
    if (q) query = query.or(`last_name.ilike.%${q}%,first_name.ilike.%${q}%,email.ilike.%${q}%,cert_number.ilike.%${q}%`);
    const [{ data, error }, totals, invitedCount, claimedCount] = await Promise.all([
      query,
      ...
      {tableMissing ? (
        ...
        <>
          <div className="mb-6 grid gap-4 sm:grid-cols-3">
            {[["Records imported", total], ["Invited to portal", invited], ["Accounts claimed", claimed]].map(([label, value]) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-muted">
                  ...
                  <th className="px-5 py-3">Expires</th><th className="px-5 py-3">Invited</th><th className="px-5 py-3">Claimed</th>
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 ? (
                  <tr><td colSpan={6} className="px-5 py-8 text-center text-muted">{q ? "No records match that search." : "No records imported yet."}</td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className="border-b border-line last:border-0">
                    ...
                    <td className="px-5 py-3">{r.claimed_by
                      ? <span className="rounded-full bg-success/10 px-2.5 py-1 text-xs font-semibold text-success">Claimed</span>
                      : <span className="rounded-full bg-bg px-2.5 py-1 text-xs font-semibold text-muted">Unclaimed</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
