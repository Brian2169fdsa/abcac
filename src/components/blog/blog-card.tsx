...
export function BlogCard({ post }: BlogCardProps) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.6rem] border border-ink/10 bg-surface shadow-[0_24px_60px_-46px_rgba(13,34,63,0.65)] transition duration-300 hover:-translate-y-1 hover:border-brand/25 hover:shadow-[0_30px_70px_-42px_rgba(13,34,63,0.55)]">
      <Link href={`/blog/${post.slug}`} className="relative block aspect-[16/10] overflow-hidden bg-info">
        <Image
        ...
        <span className="absolute bottom-4 left-4 rounded-full border border-white/20 bg-info/85 px-3 py-1 text-[0.68rem] font-bold uppercase tracking-[0.13em] text-white backdrop-blur">
          {post.category}
        </span>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h2 className="mt-4 text-xl leading-snug transition-colors group-hover:text-brand">
